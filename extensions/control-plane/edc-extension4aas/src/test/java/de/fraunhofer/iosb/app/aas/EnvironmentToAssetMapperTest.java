...
/**
 * Do not test for empty Shell, Submodel, ConceptDescription inside an environment since the default implementation of the environment class does not allow null values.
 */
class EnvironmentToAssetMapperTest {
    ...
    //                (HttpDataAddress) getChildren(result.getEnvironment(), CONCEPT_DESCRIPTIONS).stream().map(Asset::getDataAddress).toList().get
    //                (0);
    //
    ...
    //        assertEquals("concept-descriptions/%s".formatted(Base64.getEncoder().encodeToString(env.getConceptDescriptions().get(0).getId()
    //        .getBytes())),
