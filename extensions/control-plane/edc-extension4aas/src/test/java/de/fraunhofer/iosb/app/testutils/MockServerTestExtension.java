...
public class MockServerTestExtension {
    ...
    protected void mockResponse(METHOD method, String requestPath, Object responseBody, int responseStatus) throws SerializationException,
        ...
        switch (method) {
            case GET:
                mockGetResponse(requestPath, responseBody, responseStatus);
            case PUT:
                mockPutResponse(requestPath, responseBody, responseStatus);
            case DELETE:
                mockDeleteResponse(requestPath, responseStatus);
            case POST:
                mockPostResponse(requestPath, responseBody, responseStatus);
        }
    }
    ...
    private void mockGetResponse(String requestPath, Object responseBody, int responseStatus) throws SerializationException,
        ...
        server.stubFor(WireMock.get(urlPathEqualTo(requestPath))
                .willReturn(aResponse()
                        .withResponseBody(new Body(jsonApiSerializer.write(responseBody)))
    ...
    private void mockPostResponse(String requestPath, Object object, int responseStatus) throws SerializationException,
            UnsupportedModifierException {
        server.stubFor(WireMock.post(urlPathEqualTo(requestPath))
                .willReturn(aResponse()
                        .withResponseBody(new Body(jsonApiSerializer.write(object)))
                        .withStatus(responseStatus)));
    }
