...
# We can assume that fully namespaced catalog fields are equal to "aas:" prefix
verify_request "${MANAGEMENT_API}/catalog/request" "catalog_namespaced" "POST" "$catalog_request"
################################ Step 2: Check asset store ################################
# We can assume that fully namespaced assets are equal to "aas:" prefixed
verify_request "${MANAGEMENT_API}/assets/request" "assets_namespaced" "POST" "$query_spec"
...
SELF_DESCRIPTION_API="http://localhost:18281/api/selfDescription"
...
mv ./launchers/standalone/build.gradle.kts.bak ./launchers/standalone/build.gradle.kts
